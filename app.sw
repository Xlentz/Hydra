...
class HydraGame {
    constructor() {
        ...
        // Vollständige Avatarauswahl
        this.avatars = ["🐭","👻","👽","👨🏻‍💼","🥷🏼","🧙🏻","👩🏻‍💼","🦊","🐵","🐶","🦁","🐯","🐰","🐙","⚽️"];
        this.selectedAvatarTemp = "🐭"; // Temporärer Speicher für die Auswahl im Modal
        ...
        // Schutz vor alten unvollständigen Speicherständen
    ...
    /**
     * Erstellt die Avatar-Schaltflächen und verknüpft sie direkt mit der Klick-Logik
     */
    buildAvatarGrid() {
        ...
        this.avatars.forEach(av => {
            ...
            // Highlight setzen, falls es der aktuell gewählte Avatar ist
            if (av === this.selectedAvatarTemp) {
                btn.classList.add('selected');
            }
            
            // Direktes Klick-Event beim Erzeugen injizieren (Behebt den Bug)
            btn.onclick = (e) => {
    ...
    setupEventListeners() {
        ...
        document.getElementById('btn-open-profile').onclick = () => {
            ...
            // Aktuellen Avatar für die Auswahl setzen und Grid neu zeichnen
            this.selectedAvatarTemp = this.playerStats.avatar;
            this.buildAvatarGrid();
                ...
                if (cell.type === 'WALL') {
                    ...
                    if (cell.flows) {
                        if (cell.type === 'PIPE_STRAIGHT') {
                            ...
                            DIRS.forEach(dr => {
                                if(cell.flows[dr.d]) {
                                    let hex = this.colorMap[cell.flows[dr.d]];
                                    this.ctx.save();
                                    this.ctx.strokeStyle = hex; this.ctx.lineWidth = 6;
                                    this.ctx.shadowBlur = 10; this.ctx.shadowColor = hex;
